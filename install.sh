...
if [ "$CHECK_ONLY" -eq 0 ]; then
  mkdir -p "$SKILLS_DST" "$AGENTS_DST" "$PIPELINES_DST" "$WORKFLOWS_DST"
fi
...
if [ "$CHECK_ONLY" -eq 1 ]; then
  echo ""
  echo "Checked: $SKILL_COUNT skill(s), $AGENT_COUNT subagent(s), $PIPELINE_COUNT pipeline YAML(s), $WORKFLOW_COUNT workflow(s)"
